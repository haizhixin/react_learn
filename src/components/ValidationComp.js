...
export default class ValidationComp extends Component {
    ...
    static propTypes = {
        a: PropTypes.number.isRequired,//a属性必须是一个数字类并且必填
        b: PropTypes.bool.isRequired,//b必须是一个bool属性,并且必填
        onClick: PropTypes.func,//onClick必须是一个函数
        c: PropTypes.any,//1,可以设置必填,2阵型保持整齐,(所有属性都在该对象中)
        ...
        h: PropTypes.arrayOf(PropTypes.number),//数组中的每一项必须满足类型要求
        i: PropTypes.ObjectOf(PropTypes.number),//每一个属性必须满足类型要求、
        j: PropTypes.shape({//属性必须满足该对象的要求
            name: PropTypes.string.isRequired,//name必须是一个字符串,且为必填
        ...
        score: function (props, propName, componentName) {
            ...
            // 并且取值范围是0~100
            if (val < 0 || val > 100) {
                return new Error(`invalid porp ${propName} in ${componentName},${propName} msut is between 0 and 100`);
            }
        }
    }
